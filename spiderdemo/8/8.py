...
def fetch_font_data(session: httpx.Client, url: str, cookies: dict, headers: dict, params: dict) -> dict:
    """获取字体挑战数据"""
    try:
...
def save_font(b64_font: str, path: str) -> None:
    """保存 Base64 字体为 WOFF2 文件"""
    with open(path, "wb") as f:
        f.write(base64.b64decode(b64_font))
    print(f"[+] WOFF2 文件已保存: {path}")


def font_to_xml(woff2_path: str, xml_path: str) -> None:
    """解析 WOFF2 字体生成 XML"""
    try:
        font = TTFont(woff2_path)
        font.saveXML(xml_path)
        print(f"[+] 字体 XML 文件已保存: {xml_path}")
    except Exception as e:
        raise RuntimeError(f"字体解析失败: {e}")


def extract_glyphs(xml_path: str) -> dict:
    """提取 glyph 轮廓"""
    ...
    for map_elem in root.findall('.//GlyphID'):
        # print(map_elem)
        code = map_elem.get('id')  # Unicode 码点
        name = map_elem.get('name')  # glyph 名称
        # print(code)
        # print(int(code, 16))
        # print(name)

        if code and name and '.notdef' not in name:
            glyph_outlines[name] = int(code) - 1

    print(f"[+] 提取到的 glyph 数量: {len(glyph_outlines)}")
    print(f"[+] 提取到的 glyph: {glyph_outlines}")
    return glyph_outlines

def main(page):
    session = httpx.Client(http2=True)
    url = f"https://www.spiderdemo.cn/font_anti/api/font_anti_challenge/page/{page}/"
    cookies = {'sessionid': 'cjomi84jr3x0r0boau4r978b9sk0qjgy'}
    headers = {
        'accept': 'application/json, text/javascript, */*; q=0.01',
        'accept-language': 'zh-CN,zh;q=0.9',
        'cache-control': 'no-cache',
        'pragma': 'no-cache',
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/141.0.0.0 Edg/141.0.0.0',
        'x-requested-with': 'XMLHttpRequest',
    }
    params = {'challenge_type': 'font_anti_challenge'}

    res = fetch_font_data(session, url, cookies, headers, params)
    b64_font = res.get('b64Font')
    old_data = res.get('page_data')
    woff2_path = "font.woff2"
    xml_path = "font.xml"

    save_font(b64_font, woff2_path)
    font_to_xml(woff2_path, xml_path)
    glyph_outlines = extract_glyphs(xml_path)
    setkey={
        '1':'one',
        '2':'two',
        '3':'three',
    ...
    for i in old_data:
        ...
        for k in i:
            # print(k)
            forkey=setkey[k]
            xykey=glyph_outlines[forkey]
            # print(f'{k}--》{xykey}')
            newk=newk+str(xykey)
        # print(newk)

        # print('#'*10)
        new_data.append(newk)
    # print(old_data)
    # print(new_data)
    return new_data
